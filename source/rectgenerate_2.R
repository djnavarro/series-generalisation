...
rectgenerate <- function(seed) {
  ...
  n <- round(rnorm(1, mean = 30000, sd = 5000)) # number of consequential regions to sample
  ...
  pic <- ggplot(
    data = hypotheses, 
    mapping = aes(
      x0 = mid_x, y0 = mid_y, r = len_x, 
      sides = sides, angle = 0
    )
  ) + 
    geom_regon(aes(fill = len_x + len_y), 
              alpha = .15, color = "white", 
              show.legend = FALSE) + 
    
    geom_regon(fill = NA, color = paste0(bg, "22"), 
              show.legend = FALSE) + 
    
    # stylistic
    theme_void() +
    theme(plot.background = element_rect(fill = bg, colour = bg)) + 
    scale_x_continuous(NULL, labels = NULL, limits = c(-r, r)*2) + 
    scale_y_continuous(NULL, labels = NULL, limits = c(-r, r)*2) + 
